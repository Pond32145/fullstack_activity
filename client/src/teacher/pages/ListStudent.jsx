...
const ProductTable = () => {
  ...
  const [itemsPerPage, setItemsPerPage] = useState(1); // จำนวนรายการต่อหน้า
    ...
    fetch("http://localhost:3333/api/user")
      ...
      .then(
        (result) => {
          ...
          const filteredItems = result.filter((item) => item.role === "student" || item.role === "admin");
          ...
          <div className="flex space-x-2">
            {Array.from({ length: Math.ceil(filteredItems.length / itemsPerPage) }).map((_, i) => (
              <button
                ...
                className={`px-4 py-2 font-medium ${currentPage === i ? "text-blue-600 bg-blue-100" : "text-gray-600 bg-gray-100"
